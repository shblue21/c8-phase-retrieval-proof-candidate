...
The family contains the frame from the first release and, after an
invertible change of signal coordinates, yields a representative whose
defining polynomials contain twenty monomial terms in total. The result gives $m_{\CC}(8)\le26$;
...
in the first release is another member of the family.
...
The substitutions make all coefficients of $h$ below index 6, and also
$r_6$, zero. Any remaining difference starts with $i\alpha_6p_6$.
Restarting at 6 and eliminating through degree 19 yields
\[
...
vanishes. The remaining imaginary coefficients are $\alpha_4$ and $\alpha_6$.
...
release. In particular $\Delta=2>0$.
...
Conversely, if $x,y$ have equal measurements but do not differ by a
common phase, then $H=xx^*-yy^*$ is a nonzero Hermitian matrix of rank
at most two in $\ker\mathcal A_Q$. Indeed, $xx^*=yy^*$ would force $x,y$
to differ by a unit-modulus scalar, including the zero-vector case.
Thus the kernel condition and phase retrieval are equivalent.

\section{Scope and verification}\label{sec:status}
Theorem~\ref{thm:main} classifies the family \eqref{eq:assembly} in the
domain $0<s<1$, $m<0$, $\gamma>0$ and includes the original frame.
...
\input{verification_summary.tex}
...
\noindent\textbf{Version history.} The first version of this manuscript was
released on 19 September 2026 and is archived at
\href{https://doi.org/10.5281/zenodo.22847200}{doi:10.5281/zenodo.22847200}.
This revision extends that construction to the family studied here.
...
Each list assumes that the $p$-basis coefficients of $h$ below the stated
index vanish. The recurrence in Section~\ref{sec:normal} determines the
displayed components; components not yet determined remain free at that
stage. All denominators
...
\input{coefficient_substitutions.tex}
